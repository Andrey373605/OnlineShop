...
- `repositories` - доступ к данным и контракты через `Protocol`
- `core` - конфигурация, lifespan, инфраструктурные ресурсы, обработка ошибок
...
В репозитории уже есть более подробное руководство:

- `guide.md` - расширенное описание API, окружения и сценариев запуска
