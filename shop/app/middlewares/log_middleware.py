...
def _get_event_log_service(request: Request) -> EventLogService:
    """Создаёт EventLogService из app.state.mongo_client для использования в middleware."""
...
async def global_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    """Логирует необработанные исключения приложения в MongoDB через EventLogService.
    HTTPException и RequestValidationError не перехватываются — их обрабатывает FastAPI.
    """
    ...
    try:
        ...
        log.exception("Failed to log exception to event_log: %s", log_exc)
    return JSONResponse(
        status_code=500,
