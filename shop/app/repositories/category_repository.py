...
class CategoryRepositorySql:
    ...
    async def create(self, category_data: dict) -> int:
        ...
        result = await self._queries.create_category(self._conn, **category_data)
        return result["id"]
    ...
    async def update(self, category_id: int, update_data: dict) -> bool:
        ...
        result = await self._queries.update_category(
            self._conn, id=category_id, **update_data
        )
        return bool(result)

    async def delete(self, category_id: int) -> bool:
        """Возвращает успешность удаления"""
        result = await self._queries.delete_category(self._conn, id=category_id)
        return bool(result)
