...
class ProductSpecificationRepositorySql:
    ...
    async def get_all(self) -> list[ProductSpecificationOut]:
        rows = await self._queries.get_all_product_specifications(self._conn)
    ...
    async def get_by_id(self, specification_id: int) -> ProductSpecificationOut | None:
        row = await self._queries.get_product_specification_by_id(
            self._conn,
            id=specification_id,
        )
    ...
    async def create(self, data: dict) -> int:
        result = await self._queries.create_product_specification(
            self._conn,
            **data,
        )
        return result["id"]
    ...
    async def update(self, specification_id: int, data: dict) -> bool:
        result = await self._queries.update_product_specification(
            self._conn,
            id=specification_id,
            **data,
        )
    ...
    async def delete(self, specification_id: int) -> bool:
        result = await self._queries.delete_product_specification(
            self._conn,
            id=specification_id,
        )
        return bool(result)
