...
from shop.app.models.schemas.product_image_schemas import ProductImagesDeleteResult
...
class ProductImageService:
    def __init__(
        ...
        file_validator: FileValidatorPort,
        filename_generator: ObjectKeyGeneratorPort,
    ) -> None:
        self._uow = uow
    ...
    async def create_image(self, data: ProductImageCreate, source: UploadSource) -> ProductImage:
        ...
        try:
            async with self._uow as uow:
                product_image_data = ProductImage.new(
    ...
    async def delete_images_by_product_id(
        ...
        async with self._uow as uow:
            ...
            try:
                images = await uow.product_images.list_by_product_id(product_id)
                deleted_ids = await uow.product_images.delete_all_for_product(product_id)
                await uow.commit()
            except RepositoryForeignKeyError as exc:
                raise ConflictError(
                    "Product image is referenced by another entity",
